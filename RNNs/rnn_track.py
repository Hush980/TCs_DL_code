...
y_data=data_y.reshape((data_y.shape[0], 16))
...
layer_1 = SimpleRNN(128,return_sequences=True)(lstm_input)
layer_2 = SimpleRNN(64,return_sequences=False)(layer_1)
layer_3=Dense(32)(layer_2)
layer_4=Dense(16)(layer_3)
model = Model(inputs=[lstm_input], outputs=layer_4)
lr=tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=1e-3,decay_steps=2280,decay_rate=0.9)
optimizer=tf.keras.optimizers.Adam(learning_rate=lr)
model.compile(loss='mean_squared_error', optimizer='adam', metrics=['accuracy', 'mae', 'mape'])
save_to = '/content/drive/MyDrive/ctrl_RNN/ctrl_cnn_rnn_48_track.hdf5'
...
# history = model.fit([train_X],train_y,validation_data=([valid_X],valid_y) ,epochs=200,batch_size=16,  callbacks=[es],verbose=2)
...
model.load_weights('/content/drive/MyDrive/ctrl_RNN/ctrl_cnn_rnn_48_track.hdf5')
...
act_y = y_scaler1.inverse_transform(test_y)
...
#     result_T=list(map(list, zip(*result)))
    with open(path, 'w', newline='') as csvfile:
        ...
        for row in result:
            writer.writerow(row)
#
# essemble_path('content/drive/MyDrive/ctrl_RNN/result_ctrl_cnn_rnn_fore72.csv',pre_yhat)
