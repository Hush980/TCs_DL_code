...
# uv_out = TimeDistributed(Conv2D(16, kernel_size=(2,2),strides=1,activation='relu'))(uv_out)
# uv_out = TimeDistributed(BatchNormalization())(uv_out)
# uv_out = TimeDistributed(MaxPool2D(pool_size=(2,2),strides=1, padding='valid'))(uv_out)
uv=Flatten()(uv_out)
...
# sst_out = TimeDistributed(Conv2D(8, kernel_size=(2,2),strides=1,activation='relu'))(sst_input)
# sst_out = TimeDistributed(BatchNormalization())(sst_out)
# sst_out = TimeDistributed(MaxPool2D(pool_size=(2,2), strides=1, padding='valid'))(sst_out)
sst=Flatten()(sst_out)
...
# hgt_out = TimeDistributed(Conv2D(32, kernel_size=(3,4),strides=2))(hgt_input)
# hgt_out =  TimeDistributed(BatchNormalization())(hgt_out)
# hgt_out = TimeDistributed(MaxPool2D(pool_size=(2,2), strides=1, padding='valid'))(hgt_out)
p=Flatten()(p_out)
...
layer_3 = tf.concat([uv,sst,hgt,layer_2],axis=-1)
output=Dense(24,,activation='relu')(layer_3)
model = Model(inputs=[uv_input,sst_input,p_input,gru_input], outputs=output)
...
# lr_4= tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=5e-4,decay_steps=2280,decay_rate=0.92)
# lr_5= tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=8e-4,decay_steps=2280,decay_rate=0.9)
# lr_6= tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=1e-3,decay_steps=2280,decay_rate=0.9)
...
# tf.keras.optimizers.Adam(learning_rate=lr_4),
# tf.keras.optimizers.Adam(learning_rate=lr_5),
# tf.keras.optimizers.Adam(learning_rate=lr_6)]
...
model.summary()
history = model.fit([train_X_uv,train_X_sst,train_X_p,train_X],train_y,validation_data=([valid_X_uv,valid_X_sst,valid_X_p,valid_X],valid_y) ,epochs=100,batch_size=16, callbacks=[es,reduce_lr],verbose=2,workers=5)
...
model.load_weights('./ctrl_cnn_gru_72.hdf5')
tf.compat.v1.enable_eager_execution()
yhat=model([test_X_uv,test_X_sst,test_X_hgt,test_X])
...
#     result_T=list(map(list, zip(*result)))
    with open(path, 'w', newline='') as csvfile:
        ...
        for row in result:
            writer.writerow(row)
#
essemble_path('result_ctrl_cnn_gru_fore72.csv',pre_yhat)
