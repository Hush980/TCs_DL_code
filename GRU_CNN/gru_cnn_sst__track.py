...
layer_3 = tf.concat([uv,layer_2],axis=-1)
output=Dense(24,,activation='relu')(layer_3)
model = Model(inputs=[sst_input,gru_input], outputs=output)
...
model.summary()
history = model.fit([train_X_sst,train_X],train_y,validation_data=([valid_X_sst,valid_X],valid_y) ,epochs=100,batch_size=16, callbacks=[es,reduce_lr],verbose=2,workers=5)
...
model.load_weights('./cnn_gru_sst_72.hdf5')
tf.compat.v1.enable_eager_execution()
yhat=model([test_X_sst,test_X])
...
#     result_T=list(map(list, zip(*result)))
    with open(path, 'w', newline='') as csvfile:
        ...
        for row in result:
            writer.writerow(row)
#
essemble_path('result_cnn_gru_sst_fore72.csv',pre_yhat)
